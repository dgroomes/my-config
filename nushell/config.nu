...
# Java 24 is more strict with loading native libraries, and while Gradle 8.14 does support Java 24, it seems that it
# doesn't have a final word on this because we still see a warning message because Gradle tries to load some native
# thing. The warning includes:
#
#     WARNING: java.lang.System::load has been called by net.rubygrapefruit.platform.internal.NativeLibraryLoader
#
# We can allow the native access for now, but keep an eye on how Gradle evolves around this. There should be no need for
# native code. See https://github.com/gradle/gradle/issues/31625?utm_source=chatgpt.com
